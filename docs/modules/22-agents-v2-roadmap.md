# Модуль 22 — ИИ Агенты v2 (⏸ ПРИОСТАНОВЛЕНО, ждём ТЗ от юзера)

> **Статус (2026-05-16):** Иitre 1-2 в проде, Иitre 3 приостановлена. Юзер пересматривает архитектуру и готовит **собственное ТЗ**. Не стартовать новую работу по агентам без него.
>
> **Что уже выкачено:**
> - Knowledge Hub фундамент (Иitre 1) — category column + auto-classify Haiku + UI + backfill.
> - Каталог ролей + 🔍 Поисковик (Иitre 2) — agents-v2.html + 6 endpoints.
> - Юрист с Checko перенесён в **Бизнес-решения** (не в Агенты) — там его правильное место для разовой проверки по фикс-цене.
>
> **Уточнение архитектуры (2026-05-16):** ИИ Агенты v2 = переработка существующей multi-agent оркестрации (server/agents/registry.py 48 ролей + agent_runner.py ReAct + `delegate` tool + canvas), а НЕ ещё один каталог ролей поверх Solutions. Старая инфра ЖИВАЯ, нужно её упростить под предпринимателя.
