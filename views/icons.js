...
`;
  ...
  const _HINTS = {
    ...
    '/agents-modular.html': {
      ...
      text: 'Че — один личный AI-помощник, к которому подключаешь модули-агенты (Почта, Календарь, Финансы, Тренер, Питание, Копирайтер, Директолог Я.Директ, ВК Реклама и др.). Модули реально ДЕЙСТВУЮТ: отправляют письма, создают встречи в Google, пишут в твою БД — с подтверждением через карточку в чате.',
    ...
    '/creators.html': {
      ...
      text: 'Создаём бренд → AI планирует посты (TG/VK/IG/YT) → готовим тексты+картинки → автопостинг в TG и VK. Анализ соцсетей конкурентов через Perplexity. Freemium 3 поста в месяц. Можно перегенерить только текст, картинку оставить.',
      cta: 'Создать бренд →',
    ...
    '/finance.html': {
      ...
      text: 'Загружаешь CSV банка (Tinkoff/Sber/Alfa) — авто-категоризация. Или через ИИ Агента «купил кофе 200₽» → запись. Видишь баланс, топ категорий, подозрительные подписки.',
      cta: 'Загрузить CSV',
      ctaSelector: '[data-action="import-csv"], #importCsvBtn',
    },
    '/calendar.html': {
      key: 'calendar',
      title: 'Календарь',
      text: 'Подключаешь Google Calendar (OAuth) или Yandex (CalDAV) — видишь всё в одном месте. Через ИИ Агента можно создавать встречи в Google прямо из чата.',
      cta: 'Подключить календарь',
      ctaSelector: '[data-action="connect-calendar"], #connectCalBtn',
    },
    '/notes.html': {
      key: 'notes',
      title: 'Заметки',
      text: 'Личный архив с семантическим поиском по смыслу — даже если переформулировал, найдёт. Записывай мысли через Че: «запиши что Иван обещал ТЗ к пятнице».',
      cta: 'Создать заметку',
      ctaSelector: '[data-action="create-note"], #newNoteBtn',
    },
    '/services/voice.html': {
      key: 'services-voice',
      title: 'Распознавание голоса',
      text: 'Перетаскиваешь аудио (MP3/WAV/M4A/MP4) — Whisper транскрибирует в текст. До 25 МБ. 5 ₽ за файл.',
      cta: 'Понятно',
      ctaHref: null,
    },
    '/mobile.html': {
