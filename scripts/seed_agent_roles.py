...
ROLES = [
    ...
    # ⚖️ Юрист переехал в Бизнес-решения как разовое Solution
    # (scripts/seed_solution_lawyer.py) — там ему правильное место. Здесь
    # роль-обёртка была отменена 2026-05-16 после уточнения архитектуры:
    # ИИ Агенты остаются как multi-agent оркестрация (agents.html + registry),
    # разовые проверки по фикс-цене = Solutions.
]
