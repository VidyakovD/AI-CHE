...
ROLES = [
    ...
    # ── ⚖️ Юрист (Iter 3 / role #2) ─────────────────────────────────────────
    {
        "slug": "lawyer",
        "title": "Юрист",
        "icon": "⚖️",
        "short_summary": "Проверка контрагента + анализ юр-рисков по ИНН",
        "description":
            "Делает юридическую проверку контрагента: тянет реальную выписку "
            "ЕГРЮЛ/ЕГРИП через Checko.ru (статус, ОКВЭД, директор, юр.адрес, "
            "финотчётность, суды), проверяет свежую судебную практику и "
            "регуляторные изменения через Perplexity, синтезирует риск-отчёт. "
            "Подходит для due diligence новых поставщиков, проверки клиентов "
            "перед крупной сделкой, базовой юр-проверки контрагентов.",
        "base_price_kop": 25000,   # 250 ₽ за запуск (+ ~10₽ Checko + Perplexity)
        "default_model": "claude-sonnet",
        "default_kb_categories": "legal,contacts",   # подмешает базу юзера в Иitre 4
        "system_prompt":
            ...
            "Если данных мало — честно говоришь «недостаточно данных, нужны X». "
            "Не путаешь юр.лицо (ОГРН) и ИП (ОГРНИП). Не консультируешь "
            "по уголовным вопросам — только хозяйственное и коммерческое право.",
        "input_schema": [
        ...
        "orchestra": {
            ...
            "stages": [
                ...
                },
                {
                    ...
                    "stream": True,
                    "user_prompt":
                        "Ты — юрист. Собери отчёт о проверке контрагента.\n\n"
                        "**ИНН:** {field.inn}\n"
                        "**Контекст сделки:** {field.deal_context}\n"
                        ...
                        "Структура отчёта:\n"
                        "1. **Краткое резюме** (3-5 строк): можно работать / нужны уточнения / красные флаги\n"
                        "2. **Базовые сведения** (из карточки)\n"
                        "3. **Анализ рисков** по категориям:\n"
                        "   - Финансовая устойчивость\n"
                        "   - Судебная история\n"
                        "   - Налоговая дисциплина\n"
                        "   - Репутационные риски\n"
                        "   - Регуляторные риски (по нише ОКВЭД)\n"
                        "4. **Светофор сделки**: 🟢 безопасно / 🟡 с осторожностью / 🔴 не рекомендуется\n"
                        "5. **Что запросить у контрагента дополнительно** (по сделке)\n"
                        "6. **Условия в договор** которые защитят (если решено работать)\n\n"
                        "Не выдумывай факты. Не давай рекомендаций которых нет в данных. "
                        "Если по какому-то блоку информации нет — пиши «недостаточно данных»."
                        + _REPORT_FORMAT_HINT,
                },
            ],
            "final_stage": "synthesis",
