...
# HTTP-клиент для всех HTTP-вызовов внутри engine'а. Импортируется из
# server/messaging/senders.py — единый AsyncClient на оба модуля.
from server.messaging.senders import HTTP  # noqa: E402
...
# ══════════════════════════════════════════════════════════════════════════════
#  ОТПРАВКА СООБЩЕНИЙ В МЕССЕНДЖЕРЫ — вынесено в server/messaging/senders.py.
#  Импорты сохранены для backward-compat: внешний код (routes/webhook.py,
#  routes/chatbots.py, scheduler.py, тесты) делает
#  `from server.chatbot_engine import send_telegram` и продолжает работать.
# ══════════════════════════════════════════════════════════════════════════════
from server.messaging.senders import (  # noqa: E402,F401
...
from server.messaging.senders import _disable_max_bot_for_token, _get_avito_token  # noqa: F401

# Voice (Whisper/TTS) и Python sandbox тоже вынесены — re-export для legacy кода:
from server.messaging.voice import _whisper_transcribe, _tts_generate  # noqa: E402,F401
from server.sandbox import _ast_validate_python, _run_python_sandbox  # noqa: E402,F401
