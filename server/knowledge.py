...
# RAG-файлы лежат в /uploads/knowledge/ и пока НЕ учитываются в storage-биллинге
# (отдельный TODO). Поэтому лимиты намеренно умеренные:
#   - 20 файлов на бота/агента
#   - 20 МБ на файл
#   - 500 МБ суммарно на юзера (across всех его ботов и агентов)
# При выходе на новый storage-тариф эти константы можно поднять и подключить
# биллинг (kb-файлы → StoredAsset).
MAX_FILES_PER_OWNER = 20
MAX_FILE_BYTES = 20 * 1024 * 1024            # 20 МБ
MAX_TOTAL_BYTES_PER_USER = 500 * 1024 * 1024  # 500 МБ суммарно
