...
- **Бизнес-решения PRO** (multi-agent orchestra) — **40 пилотов** в новом UI с поиском, 8 chip-фильтрами (✨ Все · 🔬 Ресёрч · 📈 Маркетинг · 💼 Продажи · 📊 Стратегия · ⚖️ Юр · 💰 Финансы · 👥 HR), **группировкой по категориям при «Все»**, бейджами категории на карточке + бейджами 🔥 ХИТ / 🆕 NEW / 💎 DEEP / 🤖 PRO.
  - **🎉 v2-редизайн ЗАВЕРШЁН (40/40):** Все 40 решений имеют явную `input_schema` (форма с полями вместо textarea) + multi-stage orchestra-pipeline. Юзер заполняет 4-8 полей, бэкенд подставляет через `{field.name}`. Pipeline'ы для 1-30 переписаны (Perplexity research → Sonnet → GPT-4o полировка где нужно), для 31-40 (deep-orchestra и Perplexity-фикс) оставлены существующие сложные pipeline'ы (parallel_browse, file_extract, vision_describe, attachments) — добавлены только input_schema.
