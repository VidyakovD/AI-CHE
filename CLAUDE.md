...
- **Бизнес-решения PRO** (multi-agent orchestra) — **40 пилотов** в новом UI с поиском, 8 chip-фильтрами (✨ Все · 🔬 Ресёрч · 📈 Маркетинг · 💼 Продажи · 📊 Стратегия · ⚖️ Юр · 💰 Финансы · 👥 HR), **группировкой по категориям при «Все»**, бейджами категории на карточке + бейджами 🔥 ХИТ / 🆕 NEW / 💎 DEEP / 🤖 PRO.
  - **🚀 v2-редизайн (текущий спринт):** 20 решений (id 1-20) переписаны с явной `input_schema` (форма с конкретными полями) + multi-stage orchestra-pipeline (Perplexity research → Sonnet анализ → GPT-4o полировка). Юзер заполняет 4-8 полей, не одну textarea. Промпты получают значения через `{field.name}` placeholder. Покрыты темы: стратегия, продажи, маркетинг, HR, финансы, операции, ИИ-внедрение, ICP.
  - **Осталось переделать в v2:** 20 решений (id 21-40) — план в TODO_NEXT.md → раздел «v2-редизайн».
