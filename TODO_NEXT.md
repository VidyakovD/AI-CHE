...
См. [docs/modules/22-agents-v2-roadmap.md](docs/modules/22-agents-v2-roadmap.md).

**TL;DR:** упростить агентов до каталога 6 готовых ролей (Поисковик/Парсер/Юрист/Бухгалтер/Креатор/Автоответчик), Knowledge Hub (общая база компании, auto-classify по категориям), скилы поверх ролей. Креаторы переезжают сюда как одна из ролей. Workflow-конструктор не удаляем — прячем за `/workflow/advanced` для разработчиков.
...
**Прогресс:**
- ✅ **Итерация 1** — Knowledge Hub: category column + auto-classify (Haiku) + UI filter + override + backfill (2026-05-15, коммиты `c45ecb8` security fixes + `448c330` Iter 1)
- ✅ **Итерация 2** — Каталог ролей + первая роль 🔍 Поисковик: модели `AgentRole`/`AgentRun` + страница `/agents-v2.html` + 6 endpoints + засеяно на проде (2026-05-16, коммит `c436eae`)
- ⏸ **Итерация 3 — ПРИОСТАНОВЛЕНА.** Юзер решил подготовить ТЗ самостоятельно (2026-05-16). После уточнения архитектуры: ИИ Агенты v2 должны быть **multi-agent оркестрацией** на базе существующей библиотеки (`server/agents/registry.py` 48 ролей + `agent_runner.py` ReAct + `delegate` tool + canvas builder), а НЕ ещё одной системой ролей поверх Solutions. Разовые проверки с фикс-ценой = Бизнес-решения (Юрист уже перенесён туда коммитом `ec283f3`).
- 🔜 **Дальнейшая работа по ИИ Агентам — ждать ТЗ от юзера.** Не стартовать самостоятельно.
