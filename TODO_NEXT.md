...
**См. [docs/modules/23-project-loom-roadmap.md](docs/modules/23-project-loom-roadmap.md)** — детальная дорожная карта (mapping ТЗ юзера на код, фазы, открытые вопросы, API источники, мобильное UX).

**TL;DR:** Юзер прислал ТЗ v0.2 «Project Loom» (платформа модульных ИИ-агентов: оркестратор + LLM Router + прокачка L0→L4 + персонализация + мобильное). 50-60% инфры уже есть в AI Студии Че, надстраиваем поверх. **Перед стартом нужны ответы на 10 открытых вопросов** из секции 7 roadmap.
